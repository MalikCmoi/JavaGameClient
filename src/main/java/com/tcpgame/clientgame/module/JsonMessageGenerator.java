...
public class JsonMessageGenerator {
    ...
    public String connect(Player player) {
        PlayerAction msg = new PlayerAction("connect", player.getName());
        return gson.toJson(msg);
    }

    public String disconnect(Player player) {
        PlayerAction msg = new PlayerAction("disconnect", String.valueOf(player.getId()));
        return gson.toJson(msg);
    }

    public String canMove(Player player, int x, int y) {
        PlayerAction msg = new PlayerAction("CanMove", String.valueOf(player.getId()), x, y);
        return gson.toJson(msg);
    }

    public String canAttack(Player player, String idPlayerCible) {
        PlayerAction msg = new PlayerAction("CanAttack", String.valueOf(player.getId()), idPlayerCible);
        return gson.toJson(msg);
    }
